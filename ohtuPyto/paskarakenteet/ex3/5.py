...
class DoublyLinkedList:
    ...
    def remove(self, index):
        ...
        # Raise exception
        if index >= self._size:
            raise ValueError("List out of bounds")
        ...
        if self._size == 1:
            self._head = self._tail == None
        if not index_node.next == None:
            index_node.next.prev = index_node.prev
            if self._head == index_node:
                self._head = index_node.next
        if not index_node.prev == None:
            index_node.prev.next = index_node.next
            if self._tail == index_node:
                self._tail == index_node.prev
